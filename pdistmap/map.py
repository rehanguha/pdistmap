...
class ClusterMapper:
    ...
    The class compares lists in `Adict` with lists in `Bdict` by computing 
    their similarity scores using the KDE intersection area. The user can
    find the top `n` closest matches for each key in `Adict` from `Bdict`.
    ...
    _list_similarity(list1: list, list2: list, adjustment_factor: float,
                     bw_method: str, linespace_num: int, plot: bool) -> float
    ...
    def _check_valid_n(self, n: int) -> None:
        """
        Validate the value of n to ensure it is within the allowable range.
        ...
        ------
        ValueError
            If n is not greater than 0 or if n is greater than or equal to the size of the lists in `Adict` or `Bdict`.
        """
        if n < 1:
    ...
    def _list_similarity(self, list1: list, list2: list, adjustment_factor: float,
                          bw_method: str, linespace_num: int, plot: bool) -> float:
        """
        Calculate the similarity between two lists using KDEIntersection.
        ...
        adjustment_factor : float
            A scaling factor to adjust the KDE computation.
        bw_method : str
            The method used to calculate the bandwidth for KDE ('scott' or 'silverman').
        linespace_num : int
        ...
        -------
        float
            The similarity score calculated as the intersection area between the KDEs of the two lists.
        """
    ...
    def find_top_n_matches(self, n: int = 2, adjustment_factor: float = 0.2,
                           ...
                           plot: bool = False) -> dict:
        """
        Find the top 'n' closest matches between the lists of two dictionaries.
        ...
        adjustment_factor : float, optional
            A scaling factor to adjust the KDE computation (default is 0.2).
        bw_method : str, optional
            The method used to calculate the bandwidth for KDE ('scott' or 'silverman') (default is 'scott').
        linespace_num : int, optional
            The number of points used to compute the KDE intersection (default is 10000).
        plot : bool, optional
        ...
        for key_A, value_A in self.Adict.items():
            ...
            for key_B, value_B in self.Bdict.items():
                similarity = self._list_similarity(value_A, value_B, adjustment_factor, bw_method, linespace_num, plot)
